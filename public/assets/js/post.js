...
function renderBadEnd(wrap, cfg){
  ...
  const videoUrl = media.badEndVideo
    ? resolveAssetUrl(cfg, media.badEndVideo)
    : (base ? joinUrlLocal(base, 'blog-assets/videos/bad-end/bad-end.mp4') : '');
  const posterUrl = media.badEndPoster
    ? resolveAssetUrl(cfg, media.badEndPoster)
    : (base ? joinUrlLocal(base, 'blog-assets/videos/bad-end/bad-end-poster.jpg') : '');
...
function renderArticleBody(body, isBack){
  ...
  if(!isBack){
    return `<div class="article" style="margin-top:14px">${escapeHtml(normalized).replace(/\n/g, '<br>')}</div>`;
  }
